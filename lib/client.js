...
`;
		...
		function SortCriterion({ criterion, icon, label, sorts, onCycle, t }) {
			...
			const title = active
				? `${label} · ${direction === "asc" ? t("browse.sort.asc") : t("browse.sort.desc")} · ${t("browse.sort.priority", { priority: index + 1 })}`
			...
			return hs("button", {
				...
				children: [
					...
					active ? h("span", { className: "pm-sort-priority", children: String(index + 1) }) : null,
				],
			});
		}
			...
			return hs("div", { className: "pm-pagination", children: [
				hs("div", { className: "pm-page-controls", children: [
					...
					...pageTokens(page, pageCount).map((token) => typeof token === "number"
						? h("button", { type: "button", className: "pm-page-button", "data-current": token === page, disabled, onClick: () => setPage(token), children: String(token) }, token)
				...
				showPageSize ? hs("label", { className: "pm-page-size-control", title: t("browse.pageSize"), children: [
					h(IconFlatListOutlineRegular, { size: 14 }),
					h("select", { id: "pm-page-size", "aria-label": t("browse.pageSize"), value: pageSize, onChange: (event) => setPageSize(Number(event.target.value)), children: [20, 50, 100].map((size) => h("option", { value: size, children: String(size) }, size)) }),
				] }) : null,
			] });
		}
		...
		function PluginCard({ plugin, t }) {
			...
			const install = async () => {
				...
				try {
					...
					const result = await remote.pluginManager.installBundle(spec, { enabled: true, registry: inspection.value.registry });
		...
		function BrowseView({ t, state, query, setQuery, refresh, sorts, setSorts, stableOnly, setStableOnly, freshnessDays, setFreshnessDays, tag, setTag, dshMetadata, setDshMetadata, page, setPage, pageSize, setPageSize }) {
			...
			const setPageSizeAndReset = (value) => changePageSize(value);
			const topPager = h(Pagination, { page: currentPage, pageCount, pageSize, setPage, setPageSize: setPageSizeAndReset, disabled: state.loading, t });
			const bottomPager = h(Pagination, { page: currentPage, pageCount, pageSize, setPage, setPageSize: setPageSizeAndReset, disabled: state.loading, t, showPageSize: false });
		...
		function PluginSourcesSection({ t }) {
			...
			react.useEffect(() => {
				...
				const timer = setTimeout(() => {
					...
					rpc("browse", { query, page, pageSize, sorts, stableOnly, freshnessDays, tag, dshMetadata }, controller.signal).then((data) => {
