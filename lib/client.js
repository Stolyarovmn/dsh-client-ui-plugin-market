...
`;
		...
		function PluginCard({ plugin, t }) {
			...
			return hs("article", { className: "pm-card", children: [
				...
				hs("div", { className: "pm-meta", children: [
					...
					h("button", { type: "button", className: "pm-compat pm-compat-button", title: t("compat.check"), onClick: () => { void loadDetails(); }, children: compatibilityLabel }),
		...
		function BrowseView({ t, state, query, setQuery, refresh, sort, setSort, stableOnly, setStableOnly, page, setPage, pageSize, setPageSize }) {
		...
		function PluginSourcesSection({ t }) {
			...
			react.useEffect(() => {
				...
				const timer = setTimeout(() => {
					...
					rpc("browse", { query, page, pageSize, sort, stableOnly }, controller.signal).then((data) => {
			...
			}, [tab, query, page, pageSize, sort, stableOnly, signature, browseRevision, connectionRevision]);
