...
`;
		...
		function Pagination({ page, pageCount, pageSize, setPage, setPageSize, disabled, t, showPageSize = true }) {
			...
			const cyclePageSize = () => {
				const index = sizes.indexOf(pageSize);
				setPageSize(sizes[(index + 1) % sizes.length]);
			};
			return hs("div", { className: "pm-pagination", children: [
				hs("div", { className: "pm-page-controls", children: [
					h("button", { type: "button", className: "pm-page-button", disabled: disabled || page <= 1, onClick: () => setPage(Math.max(1, page - 1)), title: t("browse.prev"), "aria-label": t("browse.prev"), children: h(IconChevronLeftOutlineRegular, { size: 14 }) }),
					...pageTokens(page, pageCount).map((token) => typeof token === "number"
						? h("button", { type: "button", className: "pm-page-button", "data-current": token === page, disabled: false, onClick: () => setPage(token), children: String(token) }, token)
						: h("span", { className: "pm-page-ellipsis", children: "…" }, token)),
					h("button", { type: "button", className: "pm-page-button", disabled: disabled || page >= pageCount, onClick: () => setPage(Math.min(pageCount, page + 1)), title: t("browse.next"), "aria-label": t("browse.next"), children: h(IconChevronRightOutlineRegular, { size: 14 }) }),
				] }),
				showPageSize ? hs("button", { id: "pm-page-size", type: "button", className: "pm-page-size-control", onClick: cyclePageSize, disabled, title: `${t("browse.pageSize")}: ${pageSize}`, "aria-label": `${t("browse.pageSize")}: ${pageSize}`, children: [
		...
		function PluginCard({ plugin, t }) {
			...
			const install = async () => {
				if (!spec || !remote?.pluginManager || installState === "installing" || installState === "installed") return;
				...
				try {
					...
					if (inspection.value?.status !== "accepted") throw new Error(inspection.value?.reason || "Plugin spec was rejected");
					const result = await remote.pluginManager.installBundle(spec, { enabled: false, registry: inspection.value.registry });
					if (!result?.ok) throw new Error(result?.error?.message || t("install.failed"));
					if (result.value?.application === "failed") throw new Error(result.value?.error?.message || result.value?.error?.reason || t("install.failed"));
			...
			const installIcon = installState === "installed" ? h(IconCheckOutlineRegular, { size: 16 })
				: installState === "failed" ? h(IconWarningOutlineRegular, { size: 16 })
			...
			return hs("article", { className: "pm-card", children: [
				hs("div", { className: "pm-card-top", children: [
					...
					spec ? h("button", { type: "button", className: "pm-card-install", "data-state": installState, disabled: installState === "installing" || installState === "installed", onClick: install, title: installTitle, "aria-label": installTitle, children: installIcon }) : null,
				] }),
		...
		function BrowseView({ t, state, query, setQuery, refresh, sorts, setSorts, stableOnly, setStableOnly, freshnessDays, setFreshnessDays, tag, setTag, dshMetadata, setDshMetadata, page, setPage, pageSize, setPageSize }) {
			...
			return hs("section", { className: "pm-browse-view", children: [
				...
				state.loading && !state.data ? h("p", { className: "pm-notice pm-browse-state", children: t("browse.loading") }) : null,
