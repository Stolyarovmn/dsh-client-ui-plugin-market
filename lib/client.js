...
`;
		...
		function BrowseView({ t, state, query, setQuery, refresh }) {
			...
			const plugins = [...(state.data?.plugins ?? [])]
				...
				.sort((a, b) => {
					if (sort === "stars") return (b.evidence?.stars ?? 0) - (a.evidence?.stars ?? 0) || a.name.localeCompare(b.name);
					if (sort === "downloads") return (b.evidence?.downloads30d ?? 0) - (a.evidence?.downloads30d ?? 0) || a.name.localeCompare(b.name);
