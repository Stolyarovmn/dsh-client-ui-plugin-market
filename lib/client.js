...
.pm-desc{margin:0;color:var(--dsw-alias-label-secondary);font-size:13px;line-height:18px}
.pm-desc[data-clamped=true]{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;padding-right:22px}
.pm-desc-more{position:absolute;right:0;bottom:0;height:18px;padding:0 0 0 13px;border:0;background:linear-gradient(90deg,transparent 0,var(--dsw-alias-bg-layer-1) 42%);color:var(--dsw-alias-brand-primary);font:800 13px/18px inherit;cursor:pointer}
.pm-desc-more:hover{filter:brightness(1.15)}
...
`;
		...
		function PluginCard({ plugin, t }) {
			...
			const installTitle = installState === "installing" ? t("installing")
				: installState === "installed" ? t("installed")
					: installState === "invalid" ? t("install.notBundle")
						: installState === "failed" ? t("install.retry") : t("install");
			const installIcon = installState === "installed" ? h(IconCheckOutlineRegular, { size: 16 })
			...
			return hs("article", { className: "pm-card", children: [
				hs("div", { className: "pm-card-top", children: [
					...
					spec ? h("button", { type: "button", className: "pm-card-install", "data-state": installState, disabled: installState === "installing" || installState === "installed" || installState === "invalid", onClick: install, title: installTitle, "aria-label": installTitle, children: installIcon }) : null,
				] }),
				...
				command ? hs("div", { className: "pm-command-row", children: [
		...
		function CompactFilter({ id, icon, label, value, onChange, children, active = false }) {
			return hs("label", { className: "pm-compact-filter", "data-active": active, title: label, children: [
				h("span", { className: "pm-compact-filter-icon", children: icon }),
				h("select", { id, "aria-label": label, value, onChange, children }),
			] });
		}
			...
			return hs("section", { className: "pm-browse-view", children: [
				...
				(state.data?.plugins?.length ?? 0) ? hs("div", { className: "pm-browse-content", children: [
					...
					hs("div", { className: "pm-controls-row", children: [
						...
						h(CompactFilter, { id: "pm-freshness", icon: h(IconClockOutlineRegular, { size: 14 }), label: t("browse.freshness"), value: freshnessDays, active: freshnessDays > 0, onChange: (event) => changeFreshnessDays(event.target.value), children: [
							h("option", { value: 0, children: t("browse.freshness.any") }),
							h("option", { value: 30, children: "30d" }),
							h("option", { value: 90, children: "90d" }),
							h("option", { value: 365, children: "1y" }),
						] }),
						h(CompactFilter, { id: "pm-category", icon: h(IconChecklistOutlineRegular, { size: 14 }), label: t("browse.category"), value: tag, active: Boolean(tag), onChange: (event) => changeTag(event.target.value), children: [
						...
						h(CompactFilter, { id: "pm-dsh-metadata", icon: h(IconCodeOutlineRegular, { size: 14 }), label: t("browse.dshMetadata"), value: dshMetadata, active: dshMetadata !== "any", onChange: (event) => changeDshMetadata(event.target.value), children: [
							h("option", { value: "any", children: "DSH: any" }),
							h("option", { value: "declared", children: "DSH: declared" }),
							h("option", { value: "unknown", children: "DSH: unknown" }),
						] }),
