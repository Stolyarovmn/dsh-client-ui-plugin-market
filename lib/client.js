...
`;
		...
		const en = {
			...
			"browse.search": "Search plugins, packages, repositories…", "browse.refresh": "Refresh", "browse.empty": "No plugins found", "browse.sort": "Sort", "browse.sort.relevance": "Relevance", "browse.sort.stars": "Stars", "browse.sort.downloads": "Downloads", "browse.sort.name": "Name", "browse.stableOnly": "Stable only", "browse.emptyHint": "Enable a healthy source or try another search.", "browse.loading": "Loading catalogs…", "browse.error": "Catalog request failed", "browse.results": "{count} results", "copy": "Copy command", "copied": "Copied", "copy.failed": "Could not copy", "install.warning": "Review the package and source before running this third-party install command.", "readonly": "Settings are read-only.", "unavailable": "Plugin source settings are unavailable.",
		};
		const zh = {
			...
			"browse.search": "搜索插件、包、仓库…", "browse.refresh": "刷新", "browse.empty": "未找到插件", "browse.sort": "排序", "browse.sort.relevance": "相关性", "browse.sort.stars": "星标", "browse.sort.downloads": "下载量", "browse.sort.name": "名称", "browse.stableOnly": "仅稳定版", "browse.emptyHint": "启用健康的来源或尝试其他搜索。", "browse.loading": "正在加载目录…", "browse.error": "目录请求失败", "browse.results": "{count} 个结果", "copy": "复制命令", "copied": "已复制", "copy.failed": "复制失败", "install.warning": "运行此第三方安装命令前，请检查包和来源。", "readonly": "设置为只读。", "unavailable": "插件源设置不可用。", 
		};
		...
		function PluginCard({ plugin, t }) {
			...
			return hs("article", { className: "pm-card", children: [
				hs("div", { className: "pm-card-head", children: [hs("div", { children: [
					...
					plugin.evidence?.stars !== undefined ? h("span", { className: "pm-signal", title: "GitHub stars", children: `★ ${compactNumber(plugin.evidence.stars)}` }) : null,
					plugin.evidence?.downloads30d !== undefined ? h("span", { className: "pm-signal", title: "npm downloads in the last 30 days", children: `↓ ${compactNumber(plugin.evidence.downloads30d)} / 30d` }) : null,
				...
				plugin.tags?.length ? h("div", { className: "pm-tags", children: plugin.tags.map((tag) => h("span", { className: "pm-tag", children: tag }, tag)) }) : null,
				...
				command ? hs("div", { children: [hs("div", { className: "pm-install", children: [h("code", { className: "pm-command", children: command }), h("button", { type: "button", className: "pm-icon-btn", onClick: copy, title: copyState === "ok" ? t("copied") : t("copy"), "aria-label": copyState === "ok" ? t("copied") : t("copy"), children: h(CopyIcon, { done: copyState === "ok" }) })] }), h("span", { role: "status", "aria-live": "polite", style: { position: "absolute", width: 1, height: 1, overflow: "hidden", clip: "rect(0 0 0 0)" }, children: copyState === "ok" ? t("copied") : copyState === "error" ? t("copy.failed") : "" })] }) : null,
			] });
		}
		...
		function apply(ctx) {
			settingsScope = ctx.settingsScope?.bind?.({ namespace: NS });
