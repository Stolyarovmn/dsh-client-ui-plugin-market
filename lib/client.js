window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const {
			Button, Modal, Switch, Menu, StateDot,
...
`;
		...
		const en = {
			...
			"field.name": "Name", "field.type": "Type", "field.url": "Catalog / API URL", "field.urlHint": "Optional for npm and GitHub", "source.remove": "Remove", "source.share": "Share", "source.shared": "Copied", "source.enabled": "Enabled", "source.disabled": "Disabled", "source.checking": "Checking", "source.online": "Status: online", "source.packages": "{count} packages", "source.repositories": "{count} repositories", "source.ok": "Healthy", "source.error": "Unavailable", "source.duplicate": "A source with this id already exists.", "source.required": "Name is required.", "source.urlRequired": "This source type requires a catalog URL.",
			...
			return hs("article", { className: "pm-source", children: [
				hs("div", { className: "pm-source-head", children: [
					hs("div", { className: "pm-source-main", children: [
						...
						h("div", { className: "pm-source-summary", children: [
							...
							hs("div", { className: "pm-source-evidence", children: [
								...
								status === "ok" ? hs("span", { className: "pm-source-evidence-item", children: [h(PackageIcon, {}), h("span", { children: t(source.type === "github" ? "source.repositories" : "source.packages", { count }) })] }) : null,
							] }),
						] }),
					] }),
					hs("div", { className: "pm-source-buttons", children: [
						...
						h("button", { type: "button", className: "pm-source-delete", disabled: readOnly || busy, onClick: () => onRemove(source.id), title: t("source.remove"), "aria-label": t("source.remove"), children: h(TrashIcon, {}) }),
					] }),
				] }),
		...
		function PluginCard({ plugin, t }) {
			...
			const install = async () => {
				if (!spec || !remote?.pluginManager || installState === "installing" || installState === "installed" || installState === "invalid") return;
				setInstallState("installing");
				setInstallError("");
				setInstallProgress({ phase: "checking", activity: 0 });
				let disposeState;
				let disposeLog;
				try {
					...
					setInstallProgress({ phase: "starting", requestId, activity: 0 });
					if (typeof remote?.$on === "function") {
						...
						disposeLog = remote.$on("plugin-manager/install-log", (chunk) => {
							...
							setInstallProgress((current) => ({
								...current,
								phase: current.phase === "checking" || current.phase === "starting" ? "installing" : current.phase,
								activity: (current.activity ?? 0) + 1,
							}));
						});
					}
					...
					const result = await remote.pluginManager.installBundle(spec, options);
					if (!result?.ok) throw new Error(result?.error?.message || t("install.failed"));
					if (result.value?.application === "failed" || result.value?.application === "cancelled") {
						...
						throw new Error(result.value?.error?.diagnostic || result.value?.packageResult?.output || t("install.failed"));
					}
					setInstallProgress((current) => ({ ...current, phase: "done", requestId }));
			...
			const progressTitle = installProgress.phase === "checking" ? t("install.checking")
				: installProgress.phase === "starting" ? t("install.starting")
					: installProgress.phase === "installing"
						? installProgress.attempt
							? t("install.runningAttempt", { index: installProgress.attempt.index, total: installProgress.attempt.total })
							: t("install.running")
						: installProgress.phase === "applying" ? t("install.applying")
							: t("installing");
			const installTitle = installState === "installing" ? progressTitle
				: installState === "installed" ? t("installed")
					: installState === "invalid" ? t("install.notBundle")
						: installState === "failed" ? t("install.retry") : t("install");
			const installIcon = installState === "installing" ? h(StateDot, { state: "ongoing", size: 16 })
				: installState === "installed" ? h(IconCheckOutlineRegular, { size: 16 })
					: installState === "failed" || installState === "invalid" ? h(IconWarningOutlineRegular, { size: 16 })
			...
			return hs("article", { className: "pm-card", children: [
				hs("div", { className: "pm-card-top", children: [
					hs("div", { className: "pm-card-main", children: [
						...
						hs("div", { className: "pm-card-stats", children: [
							hs("span", { className: "pm-signal", title: "GitHub stars", children: [h("span", { className: "pm-star-icon", children: "★" }), h("span", { children: compactNumber(plugin.evidence?.stars ?? 0) })] }),
					...
					installAllowed ? h("button", { type: "button", className: "pm-card-install", "data-state": installState, disabled: installState === "installing" || installState === "installed" || installState === "invalid", onClick: install, title: installTitle, "aria-label": installTitle, children: installIcon }) : null,
				] }),
				plugin.description ? h(ClampedDescription, { text: plugin.description, t, onExpand: loadDetails }) : null,
				hs("div", { className: "pm-card-meta", children: [
					...
					requiresVerification ? h("span", { className: `pm-bundle-state pm-bundle-state-${installability}`, title: bundleLabel, children: bundleLabel }) : null,
				...
				dshPeers.length && !resolvedCompatibility && detailsState.status === "ready" ? h("div", { className: "pm-source-meta", children: `DSH API: ${dshPeers.map((peer) => `${peer.dependency} ${peer.range}`).join("; ")}` }) : null,
				command && installAllowed ? hs("div", { className: "pm-command-row", children: [
		...
		function BrowseView({ t, state, query, setQuery, refresh, sorts, setSorts, stableOnly, setStableOnly, freshnessDays, setFreshnessDays, tag, setTag, dshMetadata, setDshMetadata, page, setPage, pageSize, setPageSize }) {
			...
			const topPager = h(Pagination, { page: currentPage, pageCount, pageSize, setPage, setPageSize: changePageSize, disabled: false, t });
			const bottomPager = h(Pagination, { page: currentPage, pageCount, pageSize, setPage, setPageSize: changePageSize, disabled: false, t, showPageSize: false });
			...
			return hs("section", { className: "pm-browse-view", children: [
				hs("form", { className: "pm-toolbar pm-browse-toolbar", onSubmit: (event) => { event.preventDefault(); refresh(); }, children: [
					...
					h("button", { type: "submit", className: "pm-refresh-icon pm-browse-refresh", title: t("browse.refresh"), "aria-label": t("browse.refresh"), children: h(RefreshIcon, { className: state.loading ? "pm-spin" : undefined }) }),
				] }),
				...
				(state.data?.plugins?.length ?? 0) ? hs("div", { className: "pm-browse-content", children: [
					...
					hs("div", { className: "pm-controls-row", children: [
						h("button", { type: "button", className: "pm-filter-toggle", "data-active": stableOnly, title: t("browse.stableOnly"), "aria-label": t("browse.stableOnly"), onClick: () => { setPage(1); setStableOnly(!stableOnly); }, children: h(IconShieldOutlineRegular, { size: 14 }) }),
					...
					h("div", { className: "pm-results", "data-updating": state.loading, children: plugins.map((plugin) => h(PluginCard, { plugin, t }, plugin.identity?.package || plugin.identity?.repository || plugin.identity?.fallback)) }),
					h("div", { className: "pm-browse-bottomline", children: bottomPager }),
				] }) : null,
			] });
		}
		...
		function PluginSourcesSection({ t }) {
			...
			}, [tab, query, page, pageSize, JSON.stringify(sorts), stableOnly, freshnessDays, tag, dshMetadata, signature, browseRevision, connectionRevision]);
