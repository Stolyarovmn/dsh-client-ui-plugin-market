...
import { browseSources, FILTER_TAGS, githubPluginDetails, healthSources, npmPluginDetails, SOURCE_TYPES } from "./core.js";
...
function detailsRequestFrom(payload) {
	...
	if (typeof payload.package === "string" && payload.package.trim()) {
		...
		return { kind: "npm", packageName: payload.package.trim(), version: payload.version.trim() };
	}
	if (typeof payload.repository === "string" && payload.repository.trim()) {
		if (payload.repository.length > 2_048) throw new RangeError("repository details request is too long");
		return { kind: "github", repository: payload.repository.trim() };
	}
	throw new TypeError("package or repository must be provided");
}
		...
		async function dispatch(endpoint, payload, signal) {
			try {
				...
				if (endpoint === "details") {
					...
					const value = request.kind === "npm"
						? await npmPluginDetails(request.packageName, request.version, requestOptions)
						: await githubPluginDetails(request.repository, requestOptions);
					return { ok: true, value: limitRpcValue(value, maxRpcBytes) };
				}
