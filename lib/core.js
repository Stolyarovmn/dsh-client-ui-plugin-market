...
function percentileScores(plugins, metric) {
	...
	for (let index = 0; index < ranked.length; index++) {
		scores.set(ranked[index].plugin, index / denominator);
	}
	return scores;
}
...
export async function browseSources(sourceInputs, query = "", options = {}) {
	...
	let matched = searchPlugins(dedupePlugins(rows.flatMap((row) => row.plugins)), query).slice(0, options.maxTotalPlugins ?? 1_000);
	...
	// Stars and names are already available from the normalized catalogs, so
	// they can be sorted before pagination without extra requests. Exact npm
	// download sorting needs download evidence for the whole matched set and is
	// intentionally only paid for when the user explicitly chooses Downloads.
	if (sort === "downloads" || sort === "downloads-freshness" || sort === "stars-downloads") {
		// npm search already contributes monthly download evidence for registry
		// results. The downloads API fills gaps from other sources before a
		// download-aware global ranking is calculated.
		matched = await enrichNpmDownloads(matched, { ...options, maxEvidencePackages: matched.length });
	}
	matched = sortPlugins(matched, sort);

	const offset = (page - 1) * pageSize;
	let plugins = matched.slice(offset, offset + pageSize);
	if (!["downloads", "downloads-freshness", "stars-downloads"].includes(sort)) {
		// Normal browsing only enriches the visible page. This keeps the empty
		// query fast even when hundreds of scoped npm packages are discovered.
