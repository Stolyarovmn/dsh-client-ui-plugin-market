...
function npmLatestManifestUrl(source, packageName) {
	const base = npmRegistryBase(source?.url);
	return new URL(`${encodeURIComponent(packageName)}/latest`, base);
}

function npmLatestRow(manifest, packageName) {
	if (!manifest || typeof manifest !== "object" || Array.isArray(manifest)) throw new Error("npm latest manifest returned an invalid response");
	const name = text(manifest.name) ?? packageName;
	...
	if (["custom-json", "corporate", "dsh-plugin-shop", "dshplugin-app"].includes(source.type)) {
		...
		search = async (query = "") => {
			...
			const exactBatch = exactName ? [async () => {
				try {
					return normalizeCatalog([npmLatestRow(await request(npmLatestManifestUrl(source, exactName)), exactName)], source, 1);
				} catch {
					return [];
				}
			}] : [];
...
export async function browseSources(sourceInputs, query = "", options = {}) {
	...
	// A full npm package name can bypass a stale npm search index via the adapter's
	// direct /latest manifest lookup. Merge that exact result into the canonical set.
	const exactPackage = exactNpmPackageName(query);
	if (exactPackage) {
		const exactBatches = await mapConcurrent(enabled.filter((source) => source.type === "npm"), options.concurrency ?? 4, async (source) => {
			try {
				return await createAdapter(source, options).search(exactPackage);
			} catch {
				return [];
			}
		});
		discovered = dedupePlugins([...discovered, ...exactBatches.flat()]);
	}
