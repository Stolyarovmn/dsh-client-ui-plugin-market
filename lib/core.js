...
export async function browseSources(sourceInputs, query = "", options = {}) {
	...
	// Ranking uses evidence already returned by the source APIs. Multiple
	// criteria are applied lexicographically in the order selected by the user.
	matched = sortPlugins(matched, sorts);

	const offset = (page - 1) * pageSize;
	let plugins = matched.slice(offset, offset + pageSize);
	// Fill missing npm download evidence only for the visible page. This keeps
	// Browse responsive even when the federated candidate set contains thousands.
