...
function sortPlugins(plugins, sort) {
	if (sort === "stars") return [...plugins].sort((a, b) => (b.evidence?.stars ?? 0) - (a.evidence?.stars ?? 0) || a.name.localeCompare(b.name));
	if (sort === "downloads") return [...plugins].sort((a, b) => (b.evidence?.downloads30d ?? 0) - (a.evidence?.downloads30d ?? 0) || a.name.localeCompare(b.name));
	if (sort === "freshness") return [...plugins].sort((a, b) => freshnessTimestamp(b) - freshnessTimestamp(a) || a.name.localeCompare(b.name));
	if (sort === "stars-downloads") {
		const scores = compositeScores(plugins, (plugin) => plugin.evidence?.stars ?? 0, (plugin) => plugin.evidence?.downloads30d ?? 0);
		return [...plugins].sort((a, b) => (scores.get(b) ?? 0) - (scores.get(a) ?? 0) || a.name.localeCompare(b.name));
	}
	if (sort === "downloads-freshness") {
		const scores = compositeScores(plugins, (plugin) => plugin.evidence?.downloads30d ?? 0, freshnessTimestamp);
		return [...plugins].sort((a, b) => (scores.get(b) ?? 0) - (scores.get(a) ?? 0) || a.name.localeCompare(b.name));
	}
	if (sort === "name") return [...plugins].sort((a, b) => a.name.localeCompare(b.name));
	return plugins;
}
...
export async function browseSources(sourceInputs, query = "", options = {}) {
	...
	const sort = ["relevance", "stars", "downloads", "freshness", "stars-downloads", "downloads-freshness", "name"].includes(options.sort) ? options.sort : "relevance";

	// Ranking uses evidence already returned by the source APIs. npm search
	// includes monthly download counts, GitHub contributes stars and repository
	// activity, and catalogs may contribute their own evidence. We deliberately
	// avoid thousands of secondary requests just to rank a large result set.
	matched = sortPlugins(matched, sort);
