...
export function normalizePlugin(record, source) {
	...
	return {
		...
		...((verified !== undefined || stars !== undefined || downloads30d !== undefined || rating !== undefined || ratingCount !== undefined || compatibility || dshCompatibility || releasedAt || repositoryUpdatedAt || maintenanceScore !== undefined || channel) ? {
			evidence: {
		...
		async health() {
			...
			try {
				if (source.type === "github") {
					const url = new URL(source.url ?? "https://api.github.com/search/repositories");
					if (!/\/search\/repositories$/u.test(url.pathname)) url.pathname = "/search/repositories";
					url.search = "";
					url.searchParams.set("q", "topic:dsh-plugin");
					url.searchParams.set("per_page", "1");
					const manifest = await request(url);
					const count = Number.isFinite(manifest?.total_count) ? manifest.total_count : githubRows(manifest).length;
					return { ok: true, latencyMs: Date.now() - started, count };
				}
