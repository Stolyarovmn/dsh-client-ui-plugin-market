		...
		async health() {
			...
			try {
				if (source.type === "github") {
					// Report the deduplicated set this adapter can actually discover,
					// not GitHub's raw topic total (which is polluted by unrelated DSH acronyms).
...
async function enrichInstallability(plugins, options = {}) {
	const shouldVerify = options.verifyInstallability === true
		|| (options.verifyInstallability !== false && options.fetchImpl === undefined);
	if (!shouldVerify || plugins.length === 0) return plugins;

	return mapConcurrent(plugins, Math.min(options.concurrency ?? 4, 6), async (plugin) => {
