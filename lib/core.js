	...
	if (["custom-json", "corporate", "dsh-plugin-shop", "dshplugin-app"].includes(source.type)) {
		...
		search = async (query = "") => {
			...
			const batches = await Promise.all(NPM_DISCOVERY_KEYWORDS.map(async (keyword) => {
				...
				url.searchParams.set("text", [`keywords:${keyword}`, text(query)].filter(Boolean).join(" "));
...
async function enrichNpmDownloads(plugins, options = {}) {
	...
	const chunks = [];
	for (let index = 0; index < packages.length; index += 64) chunks.push(packages.slice(index, index + 64));
	const counts = new Map();
	await mapConcurrent(chunks, Math.min(options.concurrency ?? 4, 3), async (chunk) => {
		try {
			...
			for (const name of chunk) if (typeof value?.[name]?.downloads === "number") counts.set(name, value[name].downloads);
		} catch {
