...
export function normalizePlugin(record, source) {
	...
	return {
		...
		...((verified !== undefined || stars !== undefined || compatibility) ? {
			evidence: {
...
function clonePlugin(plugin) {
	return { ...plugin, sources: [...plugin.sources], versions: [...(plugin.versions ?? [])] };
}
...
export function createAdapter(sourceInput, options = {}) {
	...
	if (["custom-json", "corporate", "dsh-plugin-shop", "dshplugin-app"].includes(source.type)) {
		...
		search = async (query = "") => {
			...
			url.searchParams.set("text", ["keywords:deepseek-harness-plugin", text(query)].filter(Boolean).join(" "));
		...
		search = async (query = "") => {
			...
			const url = new URL(base);
			url.searchParams.set("q", ["deepseek harness plugin", text(query)].filter(Boolean).join(" "));
...
export async function browseSources(sourceInputs, query = "", options = {}) {
	...
	const plugins = searchPlugins(dedupePlugins(rows.flatMap((row) => row.plugins)), query).slice(0, options.maxTotalPlugins ?? 1_000);
